...
function LoadData(props) {
  ...
  const { storeContent } = LangContentCtx; // Destructure Context component so this property can be used as dependency in useEffect
  ...
  useEffect(() => {
    ...
    const getData = () => {
      ...
      fetch("./assets/content-lang.json", {
        headers: {
        ...
        .then((data) => {
          ...
          onDataLoaded(); // Tell App.js that data has been loaded, so it can re-render
        })
        .catch((error) => {
          onDataFetchError(error); // Tell App.js that there was an error encountered fetching the data, so it can re-render
        });
    };
